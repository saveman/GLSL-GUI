...
GLSLProgram::GLSLProgram(string shaderName) : handle(0), linked(false) 
{
	compileAndLinkShader(shaderName);
}

void GLSLProgram::compileAndLinkShader(string shaderName)
{
    if( ! compileShaderFromFile(shaderName + ".vs",GLSLShader::VERTEX) )
    {
        printf("Vertex shader failed to compile!\n%s",
               log().c_str());
        exit(1);
    }
    if( ! compileShaderFromFile(shaderName + ".fs",GLSLShader::FRAGMENT))
    {
